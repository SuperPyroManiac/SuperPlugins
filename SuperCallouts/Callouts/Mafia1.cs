...
internal class Mafia1 : Callout
{
    ...
    private void ConversationProcess(UIMenu sender, UIMenuItem selItem, int index)
    {
        ...
        if (selItem == _choiceYou)
            GameFiber.StartNew(delegate
            {
                _interaction.CloseAllMenus();
                _questioning.Enabled = false;
                Game.DisplaySubtitle("~b~Agent~s~: We will leave it to you then. Seems like a dangerous choice though.",
                    6000);
                _cBlip?.Delete();
                _aBlip = new Blip(_callPos.Around2D(1, 2), 30);
                _aBlip.Color = Color.Red;
    ...
    {
        CustomScenes.Mafia1.BuildScene(out _bad1, out _bad2, out _bad3, out _bad4, out _bad5, out _bad6, out _bad7,
            out _bad8, out _badCar1, out _badCar2, out _badCar3, out _badCar4);
