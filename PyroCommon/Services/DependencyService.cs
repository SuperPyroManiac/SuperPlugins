    ...
    internal bool CheckDepends()
    {
        ...
        if (Functions.GetVersion() < new Version("0.4.9"))
        {
            ...
            Game.DisplayNotification(
                ...
                $"~r~{plugName}",
                "~y~Not Loaded!",
                "Plugin is installed incorrectly! Please see the RagePluginHook.log! Visit https://dsc.PyrosFun.com for help!"
            );
            return false;
        }
        ...
        {
            if (!File.Exists(depend.Key))
                missingDepend.Add(depend.Key);
            else if (new Version(FileVersionInfo.GetVersionInfo(depend.Key).FileVersion) < new Version(depend.Value.Version))
                outdatedDepend.Add(depend.Key);
        }
