...
internal static class VersionChecker
{
    ...
    {
        var curVersion = Settings.SEVersion;
        var receivedData = string.Empty;

        try
        {
            receivedData = new WebClient().DownloadString("https://www.lcpdfr.com/applications/downloadsng/interface/api.php?do=checkForUpdates&fileId=24437&textOnly=1").Trim();
        }
        catch (WebException)
        {
            Log.Warning("Unable to check for updates! No internet or LSPDFR is down?");
        }
        if (receivedData == Settings.SEVersion) return;
